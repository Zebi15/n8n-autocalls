...
export class Autocalls implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Autocalls',
		name: 'autocalls',
		icon: 'file:autocalls.svg',
		group: ['communication'],
		version: 1,
		subtitle: '={{$parameter["operation"] + ": " + $parameter["resource"]}}',
		description: 'Make phone calls using AI assistants from Autocalls platform',
		defaults: {
			name: 'Autocalls',
		},
	...
	methods = {
		loadOptions: {
			async getOutboundAssistants(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				const credentials = await this.getCredentials('autocallsApi');
				...
				const options = {
					...
					uri: 'https://app.autocalls.ai/api/user/assistants/outbound',
					json: true,
				};
				...
				if (resource === 'call') {
					if (operation === 'make') {
						...
						const options = {
							...
							uri: 'https://app.autocalls.ai/api/user/make_call',
							body: {
					...
					if (operation === 'getAssistants') {
						const options = {
							...
							uri: 'https://app.autocalls.ai/api/user/assistants',
							json: true,
						};
